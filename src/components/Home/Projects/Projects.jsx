    ...
    <SectionWrap>
        ...
        <div className='py-20 w-full grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-12 text-theme-text-color'>
           {ProjectsData.map((data, index)=>{
               return <ProjectCards data={data[index]} id={'project'+data.id} key={index} projectImg={`${location}${data.image}`} projectName={data.name} description={data.description} stacks={data.techStack} />
           })}
        </div>
    </SectionWrap>
  )
